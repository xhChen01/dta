...
def cross_validate(X_train, y_train, n_drugs, n_targets, params, device, metrics):
  ...
  for fold, (train_idx, val_idx) in enumerate(tqdm(kf.split(X_train), desc="Cross Validation", unit="fold", total=5)):
    # 记录当前fold的开始时间
    ...
    model = MatrixFactorization(n_drugs, n_targets, params['n_factors']).to(device)
    ...
    for epoch in tqdm(range(1, params['epochs'] + 1), desc=f"Fold {fold+1} Training", unit="epoch", leave=False):
      ...
      print(f'Epoch {epoch:2d} | Train Loss: {train_loss:.4f} | Val MSE: {val_mse:.4f}')
      # 检查是否改善
    ...
    for metric in metrics:
      ...
      logging.info(f'Validation set {metric}: {final_val_result[metric]:.4f}')
...
def search_param(n_drugs, n_targets, X_train, X_test, y_train, y_test, device, metrics, param_grid, fixed_params):
  ...
  for idx, params_values in enumerate(tqdm(param_combinations, desc="Hyperparameter Search", unit="param")):
    ...
    metric_results, avg_stop_epoch = cross_validate(X_train, y_train, n_drugs, n_targets, full_params, device, metrics)
...
def final_model_train(n_drugs, n_targets, X_train, y_train,X_test, y_test, params, device, metrics):
  # 最终模型训练（同样使用早停）
  train_set = AffinityDataset(list(map(itemgetter(0), X_train)), list(map(itemgetter(1), X_train)), y_train)
  # 为DataLoader设置随机种子生成器，确保shuffle=True时结果可复现
  g = torch.Generator()
  g.manual_seed(42)
  train_loader = DataLoader(train_set, batch_size=params['batch_size'], shuffle=True, generator=g)
  test_set = AffinityDataset(list(map(itemgetter(0), X_test)), list(map(itemgetter(1), X_test)), y_test)
  test_loader = DataLoader(test_set, batch_size=params['batch_size'])
  
  final_model = MatrixFactorization(n_drugs, n_targets, params['n_factors']).to(device)
  ...
  for epoch in range(1, params['epochs'] + 1):
      ...
      print(f'Final Train Epoch {epoch} | Loss: {train_loss:.4f} | Val MSE: {val_mse:.4f}')
      
      if val_mse < best_test_mse - params['min_delta']:
          best_test_mse = val_mse
          patience_counter = 0
      ...
      if patience_counter >= params['patience']:
          print(f"\nEarly stopping at epoch {epoch}. Best Test MSE: {best_test_mse:.4f}")
          break
  # 保存最佳模型
...
def main():
  ...
  # 加载亲和度数据
  n_drugs, n_targets, X_train, X_test, y_train, y_test  = load_data(dataset)

  # 如果进行超参数搜索
  if args.is_search:
    best_params = search_param(n_drugs, n_targets, 
                               X_train, X_test, y_train, y_test,
                              device, args.metrics,
                              param_grid, fixed_params)
  else:
    best_params = args.__dict__
    metric_results, avg_stop_epoch = cross_validate(X_train, y_train, n_drugs, n_targets, best_params, device, args.metrics)
  ...
  for metric in args.metrics:
    all_results[metric] = []
  for seed in seeds:
    n_drugs, n_targets, X_train, X_test, y_train, y_test  = load_data(dataset, seed)
    results = final_model_train(n_drugs, n_targets, X_train, y_train,X_test, y_test, best_params, device, args.metrics)
    for metric in args.metrics:
      all_results[metric].append(results[metric])

  for metric in args.metrics:
    logging.info(f"{metric} Mean Results: {np.mean(all_results[metric])}")
    logging.info(f"{metric} Std Results: {np.std(all_results[metric])}")
